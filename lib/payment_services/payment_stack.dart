...
class _MyPaymentState extends State<MyPayment> {
  ...
  void initState() {
    plugin.initialize(
        publicKey:
            "pk_test_e9ace471dea0679f4db8094a1e3e6e930bc164b4"); //MypublicKey
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      key: _scaffoldKey,
      appBar: AppBar(title: const Text('Verify your payment')),
      body: Container(
                  ...
                  data: Theme.of(context).copyWith(
                    ...
                    textTheme: Theme.of(context).textTheme.copyWith(
                          bodyText2: const TextStyle(
                              color: Color.fromARGB(65, 80, 125, 200)),
                        ),
                    colorScheme: ColorScheme.fromSwatch().copyWith(
                        secondary: const Color.fromRGBO(50, 120, 80, 160)),
                  ),
                  child: Builder(
                    builder: (context) {
                      return _inProgress
  ...
  Future<String?> _fetchAccessCodeFrmServer(String reference) async {
    String url = 'http://10.0.2.2:8000/new-access-code';
    String? accessCode;
    try {
      // ignore: avoid_print
  ...
  void _verifyOnServer(String? reference) async {
    ...
    String url = 'http://10.0.2.2:8000/new-access-code/verify/$reference';
    try {
      http.Response response = await http.get(Uri.parse(url));
